...
QString chat :: generatechatReply(const QString& message)
{
    ...
    {
        reply = "I don't actually have a name. But you can call me 'My chat'.";
    }
        ...
        if (reply->error() == QNetworkReply::NoError) {
            ...
            if (!results.isEmpty()) {
                ...
                for (int i = 0; i < m_currentQuestion.options.size(); ++i)
                {
                    ...
                    ui->chatDisplayTextEdit->appendPlainText(optionNumber + ". " + optionText);
                }

                m_currentQuestion.isValid = true;
            }
            else {
                ui->chatDisplayTextEdit->appendPlainText("chat: Sorry, I couldn't fetch a trivia question.");
            }
        } else {
            QString error = reply->errorString();
            qDebug() << "Network error:" << error;
            ui->chatDisplayTextEdit->appendPlainText("chat: Sorry, I couldn't fetch a trivia question. Error: " + error);
        }
        reply->deleteLater();
    });
}
